...
# DMZ → Zot
resource "opnsense_firewall_filter" "DmzToZot" {
  enabled     = true
  sequence    = 65
  description = "Allow DMZ to Zot"
...
# DMZ → Convertx
resource "opnsense_firewall_filter" "DmzToConvertx" {
  enabled     = true
  sequence    = 66
  description = "Allow DMZ to Convertx"
...
# DMZ → Code Server
resource "opnsense_firewall_filter" "DmzToCodeServer" {
  enabled     = true
  sequence    = 67
  description = "Allow DMZ to Code Server"
...
# DMZ → Jenkins
resource "opnsense_firewall_filter" "DmzToJenkins" {
  enabled     = true
  sequence    = 68
  description = "Allow DMZ to Jenkins"
...
# DMZ → ArgoCD
resource "opnsense_firewall_filter" "DmzToArgocd" {
  enabled     = true
  sequence    = 69
  description = "Allow DMZ to ArgoCD"
